  ...
  for(int k=1;k<=Config.floors;k++){
    ...
    for(int ii=0;ii<_rows;ii++) {
      for(int j=0;j<cols;j++) {
        ...
        if(celltype==3){
          cells[i][j]=new Cell(j*s,i*s,10000.0,false);
          if(i+1<rows) cells[i+1][j]=new Cell(j*s,(i+1)*s,2000.0,false);
          if(i+2<rows) cells[i+2][j]=new Cell(j*s,(i+2)*s,2000.0,false);
          if(i+3<rows) cells[i+3][j]=new Cell(j*s,(i+3)*s,2000.0,false);
          if(i+4<rows) cells[i+4][j]=new Cell(j*s,(i+4)*s,2000.0,false);
        }
